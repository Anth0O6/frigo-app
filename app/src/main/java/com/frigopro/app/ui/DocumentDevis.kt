...
/**
 * Ce que le devis imprimé dit.
 *
 * Il produit un [DocumentImprime], que la facture produit aussi : la mise en
 * page, la pagination et le dessin sont partagés, et seuls le titre, les dates
 * et les mentions distinguent les deux documents.
 *
 ...
 * qu'ailleurs : **ce document part chez un client**.
 */
object DocumentDevis {
    ...
    fun de(
        ...
        return DocumentImprime(
            ...
            emetteur = emetteurDe(parametres, "Devis"),
    ...
    private fun dates(devis: Devis, aujourdhui: LocalDate): List<String> = buildList {
        add("Établi le ${(devis.creeLe ?: aujourdhui).format(FORMAT_DATE_DOCUMENT)}")
        devis.valableJusquau?.let { add("Valable jusqu'au ${it.format(FORMAT_DATE_DOCUMENT)}") }
    }
