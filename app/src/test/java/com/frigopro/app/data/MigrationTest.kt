...
class MigrationTest {
    ...
    @Test
    fun `une base version 9 recoit la facturation du deplacement`() {
        creerBase(
            version = 9,
            empreinte = EMPREINTE_V9,
            ddl = DDL_V9,
            insertions = listOf(
                "INSERT INTO `devis` (`id`, `numero`, `clientId`, `clientNom`, `equipementId`, " +
                    "`equipementNom`, `objet`, `statut`, `tauxTva`, `tvaOfferte`, `assujettiTva`, " +
                    "`creeLe`, `valableJusquau`, `modifieLe`) " +
                    "VALUES ('dev-1', 'DEV-2609-004', 'cl-1', 'Hôtel Bellevue', NULL, '', " +
                    "'Remplacement compresseur', 'ENVOYE', 20.0, 0, 1, '2026-09-11', NULL, 9)",
                "INSERT INTO `lignes_devis` (`id`, `devisId`, `designation`, `quantite`, " +
                    "`unite`, `prixUnitaire`, `offerte`, `rang`) " +
                    "VALUES ('lig-1', 'dev-1', 'Compresseur Copeland', 1.0, 'unité', 980.0, 0, 0)",
                "INSERT INTO `parametres` (`id`, `technicien`, `attestation`, `themeSombre`, " +
                    "`modeGants`, `chronoAuto`, `tauxHoraire`, `tauxTva`, `assujettiTva`, " +
                    "`entreprise`, `entrepriseAdresse`, `entrepriseTelephone`, `entrepriseEmail`, " +
                    "`entrepriseSiret`, `logoFichier`, `derniereSauvegardeLe`, `modifieLe`) " +
                    "VALUES (1, 'Anthony Ouvrard', 'ATT-2024-118', 1, 0, 0, 68.0, 20.0, 1, " +
                    "'FrigoPro', '12 rue des Lilas, Lyon', '0472000000', '', '', NULL, NULL, 9)",
            ),
        )

        val db = ouvrirEtMigrer()

        db.query("SELECT * FROM `trajets`").use { curseur ->
            assertEquals("aucun déplacement ne s'invente sur les devis passés", 0, curseur.count)
        }
        ...
        db.query("SELECT `designation`, `prixUnitaire`, `deplacement` FROM `lignes_devis`").use { curseur ->
            ...
            assertEquals(
                "une ligne d'avant le déplacement n'en vient pas : sinon le premier " +
                    "recalcul l'effacerait",
                0,
                curseur.getInt(2),
            )
        }
        ...
        db.query(
            ...
            assertEquals(
                "un tarif kilométrique inventé partirait chez un vrai client",
                0.0,
                curseur.getDouble(2),
                0.001,
            )
            assertEquals(0.0, curseur.getDouble(3), 0.001)
            assertEquals("et sans plancher", 0.0, curseur.getDouble(4), 0.001)
            assertEquals(
                "qui a avancé un péage s'attend à le récupérer",
                1,
                curseur.getInt(5),
            )
        }
    ...
    /** Room refuse d'ouvrir une base dont un index manque : celui des trajets aussi. */
    @Test
    fun `la table des trajets arrive avec son index`() {
        creerBase(version = 9, empreinte = EMPREINTE_V9, ddl = DDL_V9, insertions = emptyList())
        ...
        assertTrue(
            "index_trajets_devisId attendu, trouvé $index",
            index.contains("index_trajets_devisId"),
        )
    }
    ...
    /**
     * La facturation arrive : deux tables neuves, deux colonnes sur les réglages,
     * et rien de ce qui existait ne doit bouger.
     *
     * Le test porte surtout sur les **défauts** posés par la migration. Ils ne
     * sont pas cosmétiques : un délai de paiement absent rendrait toute échéance
     * incalculable sur un téléphone déjà garni, et un taux de pénalités inventé
     * partirait sur une vraie facture.
     */
    @Test
    fun `une base version 11 recoit la facturation sans rien perdre`() {
        creerBase(
            version = 11,
            empreinte = EMPREINTE_V11,
            ddl = DDL_V11,
            insertions = listOf(
                "INSERT INTO `parametres` (`id`, `technicien`, `attestation`, `themeSombre`, " +
                    "`modeGants`, `chronoAuto`, `tauxHoraire`, `tauxTva`, `assujettiTva`, " +
                    ...
                    "`entrepriseEmail`, `entrepriseSiret`, `logoFichier`, `adresseDepart`, " +
                    "`modeDeplacement`, `prixKm`, `prixHeureTrajet`, `minimumDeplacement`, " +
                    "`refacturerPeages`, `derniereSauvegardeLe`, `modifieLe`) " +
                    "VALUES (1, 'Anthony Ouvrard', 'ATT-2024-118', 1, 0, 0, 68.0, 20.0, 1, " +
                    "'FrigoPro', '12 rue des Lilas, Lyon', '0472000000', '', '', " +
                    "'logo-1.jpg', '12 rue des Lilas, Lyon', 'KM_ET_HEURE', 0.45, 35.0, " +
                    "25.0, 1, NULL, 11)",
                "INSERT INTO `interventions` (`id`, `date`, `heure`, `client`, `ville`, " +
                    ...
                    "`technicienNom`, `arriveeLe`, `demarreLe`, `cumuleS`, `numero`, " +
                    "`signatureFichier`, `signeeLe`, `modifieLe`) " +
                    "VALUES ('int-1', '2026-03-10', '09:00', 'Boucherie Morel', 'Lyon', " +
                    "NULL, 'Fuite de fluide', NULL, NULL, '', 'TERMINEE', '', 0, 60, " +
                    "NULL, '', NULL, NULL, 5400, 'INT-2603-001', NULL, NULL, 11)",
            ),
        )
        ...
        db.query("SELECT `delaiPaiementJours`, `tauxPenalitesRetard` FROM `parametres`")
            .use { curseur ->
                ...
                assertEquals(
                    "zéro veut dire « non fixé » : c'est alors le taux légal qui s'applique",
                    0.0,
                    curseur.getDouble(1),
                    0.001,
                )
        ...
        db.query("SELECT `numero`, `cumuleS` FROM `interventions`").use { curseur ->
            assertTrue("la tournée déjà saisie est intacte", curseur.moveToFirst())
            assertEquals("INT-2603-001", curseur.getString(0))
            assertEquals(5400, curseur.getInt(1))
        }
    ...
    /**
     * Room refuse d'ouvrir une base dont un index manque, et les trois index des
     * factures sont dans ce cas : sans eux, la migration passe et l'ouverture
     * suivante échoue.
     */
    @Test
    fun `les tables de facturation arrivent avec leurs index`() {
        creerBase(version = 11, empreinte = EMPREINTE_V11, ddl = DDL_V11, insertions = emptyList())
        ...
        listOf(
            "index_factures_clientId",
            "index_factures_interventionId",
            "index_factures_devisId",
            "index_lignes_facture_factureId",
        ).forEach { attendu ->
            assertTrue("$attendu attendu, trouvé $index", index.contains(attendu))
        }
    }
    ...
    /**
     * Le magasin arrive : trois tables, et rien de ce qui existait ne bouge.
     *
     * Le test porte sur la **contrainte d'unicité** autant que sur les tables.
     * Deux lignes « atelier » pour le même article donneraient deux comptes
     * contradictoires, et rien à l'écran ne dirait lequel est le bon : c'est la
     * base qui doit le refuser, pas le dépôt qui doit y penser.
     */
    @Test
    fun `une base version 13 recoit le magasin sans rien perdre`() {
        creerBase(
            version = 13,
            empreinte = EMPREINTE_V13,
            ddl = DDL_V13,
            insertions = listOf(
                "INSERT INTO `clients` (`id`, `nom`, `ville`, `adresse`, `telephone`, " +
                    "`parentId`, `modifieLe`) VALUES ('cl-1', 'Boucherie Morel', 'Lyon', " +
                    "'3 rue Garibaldi', '0478000000', NULL, 13)",
            ),
        )

        val db = ouvrirEtMigrer()

        db.query("SELECT `nom` FROM `clients`").use { curseur ->
            assertTrue("le carnet est intact", curseur.moveToFirst())
            assertEquals("Boucherie Morel", curseur.getString(0))
        }
        ...
        db.execSQL(
            "INSERT INTO `articles` (`id`, `reference`, `designation`, `fournisseurId`, " +
                "`fournisseurNom`, `prixAchat`, `prixVente`, `unite`, `modifieLe`) " +
                "VALUES ('a-1', 'DET-42', 'Détendeur', NULL, '', 40.0, 90.0, 'u', 14)",
        )
        db.execSQL(
            "INSERT INTO `stocks` (`id`, `articleId`, `lieu`, `quantite`, `minimum`, " +
                "`modifieLe`) VALUES ('s-1', 'a-1', 'ATELIER', 3.0, 1.0, 14)",
        )
        val doublon = runCatching {
            db.execSQL(
                "INSERT INTO `stocks` (`id`, `articleId`, `lieu`, `quantite`, `minimum`, " +
                    "`modifieLe`) VALUES ('s-2', 'a-1', 'ATELIER', 9.0, 1.0, 14)",
            )
        }
        assertTrue(
            "un article ne peut avoir qu'une ligne de stock par endroit",
            doublon.isFailure,
        )
    ...
    /**
     * Les cinq index du magasin sont aussi obligatoires que ses tables : Room
     * valide le schéma entier à l'ouverture, et une migration qui passe sans eux
     * ferait échouer le démarrage suivant.
     */
    @Test
    fun `le magasin arrive avec ses cinq index`() {
        creerBase(version = 13, empreinte = EMPREINTE_V13, ddl = DDL_V13, insertions = emptyList())
        ...
        listOf(
            "index_fournisseurs_categorie",
            "index_articles_fournisseurId",
            "index_articles_reference",
            "index_stocks_articleId_lieu",
            "index_stocks_articleId",
        ).forEach { attendu ->
            assertTrue("$attendu attendu, trouvé $index", index.contains(attendu))
        }
    }
    ...
    /**
     * Les paliers dégressifs arrivent : une table, un index, rien d'autre.
     *
     * Le catalogue déjà tarifé ne doit pas bouger — un palier n'est pas une
     * remise rétroactive, et une prestation sans palier reste au plein tarif.
     */
    @Test
    fun `une base version 14 recoit les paliers sans retarifer le catalogue`() {
        creerBase(
            version = 14,
            empreinte = EMPREINTE_V14,
            ddl = DDL_V14,
            insertions = listOf(
                "INSERT INTO `prestations` (`id`, `designation`, `categorie`, " +
                    "`prixUnitaire`, `unite`, `parUnite`, `rang`, `modifieLe`) " +
                    "VALUES ('p-1', 'Maintenance split', 'MAINTENANCE', 100.0, 'u', 1, 1, 14)",
            ),
        )

        val db = ouvrirEtMigrer()

        db.query("SELECT `designation`, `prixUnitaire` FROM `prestations`").use { curseur ->
            assertTrue("le catalogue est intact", curseur.moveToFirst())
            assertEquals("Maintenance split", curseur.getString(0))
            assertEquals(100.0, curseur.getDouble(1), 0.001)
        }
        ...
        assertTrue(
            "index_paliers_prestation_prestationId attendu, trouvé $index",
            index.contains("index_paliers_prestation_prestationId"),
        )
    ...
    /**
     * Ce qu'une intervention coûte : trois colonnes, et rien qui bouge.
     *
     * Le point du test est le **défaut à zéro** de `coutHoraireInterne`. Zéro
     * veut dire « non renseigné » et non « une heure ne coûte rien » : une
     * valeur inventée ici donnerait une marge fausse, et une marge fausse se
     * paie sur la tarification de l'année suivante.
     */
    @Test
    fun `une base version 15 recoit les couts sans en inventer`() {
        creerBase(
            version = 15,
            empreinte = EMPREINTE_V15,
            ddl = DDL_V15,
            insertions = listOf(
                "INSERT INTO `pieces_posees` (`id`, `interventionId`, `designation`, " +
                    "`reference`, `quantite`, `prixUnitaire`, `modifieLe`) " +
                    "VALUES ('pp-1', 'int-1', 'Détendeur', 'DET-42', 1.0, 90.0, 15)",
        ...
        db.query("SELECT `prixUnitaire`, `prixAchat` FROM `pieces_posees`").use { curseur ->
            assertTrue("la pièce déjà posée est intacte", curseur.moveToFirst())
            assertEquals(90.0, curseur.getDouble(0), 0.001)
            assertEquals(
                "le prix d'achat n'est pas déduit du prix de vente",
                0.0,
                curseur.getDouble(1),
                0.001,
            )
    ...
    fun `une base version 16 recoit le cout du materiel sans l'inventer`() {
        creerBase(
            version = 16,
            empreinte = EMPREINTE_V16,
            ddl = DDL_V16,
            insertions = listOf(
                "INSERT INTO `devis` (`id`, `numero`, `clientId`, `clientNom`, `equipementId`, " +
                    "`equipementNom`, `objet`, `statut`, `tauxTva`, `tvaOfferte`, " +
                    "`assujettiTva`, `creeLe`, `valableJusquau`, `modifieLe`) " +
                    "VALUES ('d-1', 'DEV-2603-007', NULL, 'Boucherie Morel', NULL, '', " +
                    "'Pose clim', 'ACCEPTE', 20.0, 0, 1, '2026-03-12', '2026-04-12', 16)",
                "INSERT INTO `lignes_devis` (`id`, `devisId`, `designation`, `quantite`, " +
                    "`unite`, `prixUnitaire`, `offerte`, `deplacement`, `rang`) " +
                    "VALUES ('ld-1', 'd-1', 'Split mural 3,5 kW', 1.0, 'u', 890.0, 0, 0, 0)",
                "INSERT INTO `factures` (`id`, `numero`, `clientId`, `clientNom`, " +
                    "`clientAdresse`, `devisId`, `interventionId`, `equipementNom`, `objet`, " +
                    "`statut`, `tauxTva`, `tvaOfferte`, `assujettiTva`, `tauxPenalites`, " +
                    "`emiseLe`, `echeanceLe`, `payeeLe`, `relanceeLe`, `modifieLe`) " +
                    "VALUES ('f-1', 'FAC-2026-0001', NULL, 'Boucherie Morel', '', NULL, NULL, " +
                    "'', 'Pose clim', 'EMISE', 20.0, 0, 1, 0.0, '2026-03-12', '2026-04-11', " +
                    "NULL, NULL, 16)",
                "INSERT INTO `lignes_facture` (`id`, `factureId`, `designation`, `quantite`, " +
                    "`unite`, `prixUnitaire`, `offerte`, `rang`) " +
                    "VALUES ('lf-1', 'f-1', 'Split mural 3,5 kW', 1.0, 'u', 890.0, 0, 0)",
                // La ligne unique des réglages : même raison qu'en version 15,
                // interroger une table vide ne dirait rien de la colonne ajoutée.
                "INSERT INTO `parametres` (`id`, `technicien`, `attestation`, `themeSombre`, " +
                    "`modeGants`, `chronoAuto`, `tauxHoraire`, `coutHoraireInterne`, " +
                    "`tauxTva`, `assujettiTva`, `entreprise`, `entrepriseAdresse`, " +
                    "`entrepriseTelephone`, `entrepriseEmail`, `entrepriseSiret`, " +
                    "`logoFichier`, `adresseDepart`, `modeDeplacement`, `prixKm`, " +
                    "`prixHeureTrajet`, `minimumDeplacement`, `refacturerPeages`, " +
                    "`delaiPaiementJours`, `tauxPenalitesRetard`, `derniereSauvegardeLe`, " +
                    "`modifieLe`) " +
                    "VALUES (1, 'Anthony Ouvrard', '', 1, 0, 0, 68.0, 42.0, 20.0, 1, '', '', " +
                    "'', '', '', NULL, '', 'KM_ET_HEURE', 0.45, 35.0, 25.0, 1, 30, 0.0, " +
                    "NULL, 16)",
            ),
        )
        ...
        db.query("SELECT `prixUnitaire`, `prixAchat` FROM `lignes_devis`").use { curseur ->
            assertTrue("la ligne déjà chiffrée est intacte", curseur.moveToFirst())
            assertEquals(890.0, curseur.getDouble(0), 0.001)
            assertEquals(
                "le coût n'est pas déduit du prix de vente",
                0.0,
                curseur.getDouble(1),
                0.001,
            )
        ...
        db.query("SELECT `prixUnitaire`, `prixAchat` FROM `lignes_facture`").use { curseur ->
            assertTrue("la facture émise ne bouge pas", curseur.moveToFirst())
            assertEquals(890.0, curseur.getDouble(0), 0.001)
            assertEquals(0.0, curseur.getDouble(1), 0.001)
        }

        db.query(
            "SELECT `coefficientMateriel`, `coutHoraireInterne` FROM `parametres`",
        ).use { curseur ->
            assertTrue("les réglages déjà saisis survivent", curseur.moveToFirst())
            assertEquals(
                "zéro veut dire « non réglé », et non un coefficient qui viderait les prix",
                0.0,
                curseur.getDouble(0),
                0.001,
            )
            assertEquals(
                "et le coût horaire n'a pas bougé",
                42.0,
                curseur.getDouble(1),
                0.001,
            )
    ...
    private companion object {
        ...
        const val EMPREINTE_V13 = "5867beeef8a4e189af18e6bffbf06967"
        ...
        val DDL_V9: List<String> = listOf(
            ...
            "CREATE TABLE IF NOT EXISTS `parametres` (`id` INTEGER NOT NULL, `technicien` TEXT " +
                ...
                "TEXT, `derniereSauvegardeLe` INTEGER, `modifieLe` INTEGER NOT NULL, PRIMARY " +
                "KEY(`id`))",
            "CREATE TABLE IF NOT EXISTS `techniciens` (`id` TEXT NOT NULL, `nom` TEXT NOT NULL, " +
                "`modifieLe` INTEGER NOT NULL, PRIMARY KEY(`id`))",
        ...
        val DDL_V15: List<String> = listOf(
            "CREATE TABLE IF NOT EXISTS `interventions` (`id` TEXT NOT NULL, `date` TEXT NOT " +
                "NULL, `heure` TEXT NOT NULL, `client` TEXT NOT NULL, `ville` TEXT NOT NULL, `typeId` " +
                "TEXT, `typeLibelle` TEXT NOT NULL, `clientId` TEXT, `clientFactureId` TEXT, " +
                "`clientFactureNom` TEXT NOT NULL, `equipementId` TEXT, `equipementNom` TEXT NOT " +
                "NULL, `statut` TEXT NOT NULL, `notes` TEXT NOT NULL, `urgente` INTEGER NOT NULL, " +
                "`dureeMin` INTEGER NOT NULL, `technicienId` TEXT, `technicienNom` TEXT NOT NULL, " +
                "`numero` TEXT NOT NULL, `signatureFichier` TEXT, `signeeLe` INTEGER, `modifieLe` " +
                "INTEGER NOT NULL, `arriveeLe` INTEGER, `demarreLe` INTEGER, `cumuleS` INTEGER NOT " +
                "NULL, PRIMARY KEY(`id`))",
            "CREATE INDEX IF NOT EXISTS `index_interventions_date` ON `interventions` (`date`)",
            "CREATE INDEX IF NOT EXISTS `index_interventions_technicienId` ON `interventions` " +
                "(`technicienId`)",
            ...
            "CREATE INDEX IF NOT EXISTS `index_lignes_facture_factureId` ON `lignes_facture` " +
                "(`factureId`)",
            "CREATE TABLE IF NOT EXISTS `fournisseurs` (`id` TEXT NOT NULL, `nom` TEXT NOT NULL, " +
                "`categorie` TEXT NOT NULL, `telephone` TEXT NOT NULL, `email` TEXT NOT NULL, " +
                "`adresse` TEXT NOT NULL, `ville` TEXT NOT NULL, `siteCatalogue` TEXT NOT NULL, " +
                "`prefere` INTEGER NOT NULL, `notes` TEXT NOT NULL, `modifieLe` INTEGER NOT NULL, " +
                "PRIMARY KEY(`id`))",
            "CREATE INDEX IF NOT EXISTS `index_fournisseurs_categorie` ON `fournisseurs` " +
                "(`categorie`)",
            "CREATE TABLE IF NOT EXISTS `articles` (`id` TEXT NOT NULL, `reference` TEXT NOT " +
                "NULL, `designation` TEXT NOT NULL, `fournisseurId` TEXT, `fournisseurNom` TEXT NOT " +
                "NULL, `prixAchat` REAL NOT NULL, `prixVente` REAL NOT NULL, `unite` TEXT NOT NULL, " +
                "`modifieLe` INTEGER NOT NULL, PRIMARY KEY(`id`))",
            "CREATE INDEX IF NOT EXISTS `index_articles_fournisseurId` ON `articles` " +
                "(`fournisseurId`)",
            "CREATE INDEX IF NOT EXISTS `index_articles_reference` ON `articles` (`reference`)",
            "CREATE TABLE IF NOT EXISTS `stocks` (`id` TEXT NOT NULL, `articleId` TEXT NOT NULL, " +
                "`lieu` TEXT NOT NULL, `quantite` REAL NOT NULL, `minimum` REAL NOT NULL, `modifieLe` " +
                "INTEGER NOT NULL, PRIMARY KEY(`id`))",
        ...
        val DDL_V14: List<String> = listOf(
            "CREATE TABLE IF NOT EXISTS `interventions` (`id` TEXT NOT NULL, `date` TEXT NOT " +
                "NULL, `heure` TEXT NOT NULL, `client` TEXT NOT NULL, `ville` TEXT NOT NULL, `typeId` " +
                "TEXT, `typeLibelle` TEXT NOT NULL, `clientId` TEXT, `clientFactureId` TEXT, " +
                "`clientFactureNom` TEXT NOT NULL, `equipementId` TEXT, `equipementNom` TEXT NOT " +
        ...
        val DDL_V13: List<String> = listOf(
            "CREATE TABLE IF NOT EXISTS `interventions` (`id` TEXT NOT NULL, `date` TEXT NOT " +
                "NULL, `heure` TEXT NOT NULL, `client` TEXT NOT NULL, `ville` TEXT NOT NULL, `typeId` " +
                "TEXT, `typeLibelle` TEXT NOT NULL, `clientId` TEXT, `clientFactureId` TEXT, " +
                "`clientFactureNom` TEXT NOT NULL, `equipementId` TEXT, `equipementNom` TEXT NOT " +
        ...
        val DDL_V11: List<String> = listOf(
            ...
            "CREATE TABLE IF NOT EXISTS `parametres` (`id` INTEGER NOT NULL, `technicien` TEXT " +
                ...
                "TEXT, `adresseDepart` TEXT NOT NULL, `modeDeplacement` TEXT NOT NULL, `prixKm` REAL " +
                "NOT NULL, `prixHeureTrajet` REAL NOT NULL, `minimumDeplacement` REAL NOT NULL, " +
                "`refacturerPeages` INTEGER NOT NULL, `derniereSauvegardeLe` INTEGER, `modifieLe` " +
                "INTEGER NOT NULL, PRIMARY KEY(`id`))",
            "CREATE TABLE IF NOT EXISTS `techniciens` (`id` TEXT NOT NULL, `nom` TEXT NOT NULL, " +
                "`modifieLe` INTEGER NOT NULL, PRIMARY KEY(`id`))",
            "CREATE TABLE IF NOT EXISTS `points_checklist` (`id` TEXT NOT NULL, `interventionId` " +
        ...
        /** Le schéma de la version 16, repris mot pour mot de `app/schemas/16.json`. */
        val DDL_V16: List<String> = listOf(
            ...
            "CREATE TABLE IF NOT EXISTS `equipements` (`id` TEXT NOT NULL, `clientId` " +
                ...
                " `modele` TEXT NOT NULL, `numeroSerie` TEXT NOT NULL, `fluide` TEXT NOT " +
                "NULL, `chargeKg` REAL, `misEnServiceLe` TEXT, `dernierControleLe` TEXT, " +
                "`modifieLe` INTEGER NOT NULL, PRIMARY KEY(`id`))",
            "CREATE INDEX IF NOT EXISTS `index_equipements_clientId` ON `equipements` " +
                "(`clientId`)",
            "CREATE INDEX IF NOT EXISTS `index_equipements_parentId` ON `equipements` " +
            ...
            "CREATE TABLE IF NOT EXISTS `lignes_devis` (`id` TEXT NOT NULL, `devisId` " +
                ...
                "NULL, `deplacement` INTEGER NOT NULL, `rang` INTEGER NOT NULL, PRIMARY " +
                "KEY(`id`))",
            "CREATE INDEX IF NOT EXISTS `index_lignes_devis_devisId` ON `lignes_devis` " +
                "(`devisId`)",
            "CREATE TABLE IF NOT EXISTS `parametres` (`id` INTEGER NOT NULL, `technicien`" +
                ...
                "`derniereSauvegardeLe` INTEGER, `modifieLe` INTEGER NOT NULL, PRIMARY " +
                "KEY(`id`))",
            ...
            "CREATE TABLE IF NOT EXISTS `trajets` (`id` TEXT NOT NULL, `devisId` TEXT NOT" +
                " NULL, `depart` TEXT NOT NULL, `arrivee` TEXT NOT NULL, `distanceKm` REAL " +
                "NOT NULL, `dureeMinutes` INTEGER NOT NULL, `peages` REAL NOT NULL, " +
